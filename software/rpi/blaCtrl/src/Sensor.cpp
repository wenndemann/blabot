...
void Sensor::setMeasuringInterval(int intervalMs) {
	Thread2Ptr t = &Sensor::callbackTimer;
...
void* Sensor::callbackTimer(void* arg)
{
	boost::asio::io_service io;
	boost::asio::deadline_timer t(io, boost::posix_time::milliseconds(m_intervalMs));
	t.async_wait(boost::bind(&Sensor::readDataSensor, this, &t));
	io.run();
	return NULL;
}

void Sensor::readDataSensor(boost::asio::deadline_timer* t) {
	uint8_t temp;
	...
	if(m_intervalMs) {
		pthread_mutex_lock(m_mutex);
			...
			for (int i = 0; i < 3; i++) { swapInt16(m_sensorData.gyro[i]); }
			...
			for (int i = 0; i < 3; i++) { swapInt16(m_sensorData.mag[i]); }
...
void Sensor::swapInt16(int16_t &inout) {
